...
abstract public class UiLocalization extends MtfAwareLocalization
{
		...
		throws IOException 
	{
		...
		writer.writeln("# 10d. In CreateCustomFieldsHelp2/3, please change the English location examples (e.g. California, CA.LA) to something sort of generic  that would make sense in your language (e.g. you could change  the translation of \"State\" to \"Region\" if it makes more sense, or even to \"Country\" if your language is spoken in more than one.  If we don't want to point out a particular country for political reasons, you could make the entries be something like the translation of \"State 1\" \"State 2, and \"City1\" \"City 2\") as long as there are 3 top level entries each with 2 entries underneath.  Make sure that whatever you change is consistent in all the places it is used:  <UseReusableChoices code='StateChoices'>, ><Label>State of Birth</Label>, <ReusableChoices code='StateChoices' label='State'>"); 		
